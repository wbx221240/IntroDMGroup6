...
import pandas as pd
from utils.configs import records
...
def evaluate(truth, score):
    """Evaluate the AUROC, F-score, precision, recall

    Args:
        truth (np.ndarray): The truth labels for time series segments
        score (np.ndarray): The anomaly score given by the models.
    """
    # setting the threshold of anomaly. 
    assert truth.shape[0] == score.shape[0]
    scaler = MinMaxScaler()
    score = scaler.fit_transform(score.reshape(-1, 1))
    topK = int(np.sum(truth))
    topK_indice = np.argsort(score.flatten())[-topK:]
    pred = np.zeros_like(truth)
    pred[topK_indice] = 1
    auc = roc_auc_score(truth, score)
    precision, recall, f_score, support = precision_recall_fscore_support(truth, pred, average='binary')
    return precision, recall, f_score, auc


def result2df(result, save_path):
    df = pd.DataFrame(result, columns=records)
    df.to_csv(save_path)
...
def get_label(X, cycle, anomaly_list, datafile):
    # print(X.shape, np.max(anomaly_list))
    if "MBA" in datafile:
        window_num = int(X.shape[0] / cycle)
        label_list = np.zeros(window_num)
        j = 0
        for idx, i in enumerate(list(range(0, X.shape[0], cycle))):
            if j == len(anomaly_list):
                break
            if anomaly_list[j] >= i and anomaly_list[j] < i + cycle:
                label_list[idx] = 1
                j += 1
    else:
        window_num = (int)(X.shape[0] / cycle)
